...
export class SeederService {
  ...
  private async createStepsSeed() {
    for (const initialStep of INITIAL_STEPS) {
      const existingStep = await this.stepService.findOneBylevelAndFlowId(initialStep.level, initialStep.flowId)
  ...
  private hasStepChanged(existingStep: any, initialStep: any): boolean {
    ...
    if (existingStep.expectedResponse !== initialStep.expectedResponse) return true
    if (existingStep.expectedResponseType !== initialStep.expectedResponseType) return true
    ...
    for (let i = 0; i < initialMessages.length; i++) {
      const existing = existingMessages.find(m => m.errorType === initialMessages[i].errorType)
